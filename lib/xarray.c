...
static inline u64 *node_marks(struct xa_node *node, xa_mark_t mark)
{
...
static void xas_squash_marks(const struct xa_state *xas)
{
	...
	do {
		u64 *marks = xas->xa_node->marks[mark];
...
static unsigned int get_offset(u64 index, struct xa_node *node)
{
...
static void xas_move_index(struct xa_state *xas, u64 offset)
{
 ...
 */
static u64 xas_max(struct xa_state *xas)
{
	u64 max = xas->xa_index;

	return max;
}

/* The maximum index that can be contained in the array without expanding it */
static u64 max_index(void *entry)
{
	if (!xa_is_node(entry))
		return 0;
 ...
 */
void *xas_find(struct xa_state *xas, u64 max)
{
	void *entry;
 ...
 * Return: The entry, if found, otherwise %NULL.
 */
void *xas_find_marked(struct xa_state *xas, u64 max, xa_mark_t mark)
{
	bool advance = true;
	unsigned int offset;
	void *entry;
 ...
 * Return: The old entry at this index.
 */
void *__xa_erase(struct xarray *xa, u64 index)
{
	XA_STATE(xas, xa, index);
 ...
 */
void *__xa_store(struct xarray *xa, u64 index, void *entry, gfp_t gfp)
{
	XA_STATE(xas, xa, index);
	void *curr;
 ...
 * failed.
 */
void *xa_store(struct xarray *xa, u64 index, void *entry, gfp_t gfp)
{
	void *curr;
 ...
 * Return: The old entry at this index or xa_err() if an error happened.
 */
void *__xa_cmpxchg(struct xarray *xa, u64 index,
 ...
 */
int __xa_insert(struct xarray *xa, u64 index, void *entry, gfp_t gfp)
{
	XA_STATE(xas, xa, index);
	void *curr;
 ...
 */
int __xa_reserve(struct xarray *xa, u64 index, gfp_t gfp)
{
	XA_STATE(xas, xa, index);
	void *curr;
 ...
 */
void __xa_set_mark(struct xarray *xa, u64 index, xa_mark_t mark)
{
	XA_STATE(xas, xa, index);
 ...
 * Context: Any context.  Expects xa_lock to be held on entry.
 */
void __xa_clear_mark(struct xarray *xa, u64 index, xa_mark_t mark)
{
	XA_STATE(xas, xa, index);
	void *entry = xas_load(&xas);
 ...
 */
bool xa_get_mark(struct xarray *xa, u64 index, xa_mark_t mark)
{
	XA_STATE(xas, xa, index);
	void *entry;
 ...
 */
void xa_set_mark(struct xarray *xa, u64 index, xa_mark_t mark)
{
	xa_lock(xa);
 ...
 * Context: Process context.  Takes and releases the xa_lock.
 */
void xa_clear_mark(struct xarray *xa, u64 index, xa_mark_t mark)
{
	xa_lock(xa);
 ...
 * Return: The entry, if found, otherwise %NULL.
 */
void *xa_find(struct xarray *xa, u64 *indexp,
			u64 max, xa_mark_t filter)
{
 ...
 */
void *xa_find_after(struct xarray *xa, u64 *indexp,
			u64 max, xa_mark_t filter)
{
...
static unsigned int xas_extract_marked(struct xa_state *xas, void **dst,
			u64 max, unsigned int n, xa_mark_t mark)
{
	void *entry;
	unsigned int i = 0;
 ...
 */
unsigned int xa_extract(struct xarray *xa, void **dst, u64 start,
			u64 max, unsigned int n, xa_mark_t filter)
{
