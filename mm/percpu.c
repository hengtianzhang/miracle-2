...
#define __addr_to_pcpu_ptr(addr)					\
	(void __percpu *)((u64)(addr) -			\
			  (u64)pcpu_base_addr	+		\
			  (u64)__per_cpu_start)
#endif
#ifndef __pcpu_ptr_to_addr
#define __pcpu_ptr_to_addr(ptr)						\
	(void __force *)((u64)(ptr) +				\
			 (u64)pcpu_base_addr -		\
			 (u64)__per_cpu_start)
#endif
...
const u64 *pcpu_unit_offsets __ro_after_init;	/* cpu -> unit offset */

/* group information, used for vm allocation */
static int pcpu_nr_groups __ro_after_init;
static const u64 *pcpu_group_offsets __ro_after_init;
 ...
 * cpu_map array which is s64 enough for @nr_units and filled with
...
static void pcpu_next_unpop(u64 *bitmap, int *rs, int *re, int end)
{
	*rs = find_next_zero_bit(bitmap, end, *rs);
	*re = find_next_bit(bitmap, end, *rs + 1);
}

static void pcpu_next_pop(u64 *bitmap, int *rs, int *re, int end)
{
 ...
 */
static u64 *pcpu_index_alloc_map(struct pcpu_chunk *chunk, int index)
{
	return chunk->alloc_map +
	       (index * PCPU_BITMAP_BLOCK_BITS / BITS_PER_LONG);
}

static u64 pcpu_off_to_block_index(int off)
{
	return off / PCPU_BITMAP_BLOCK_BITS;
}

static u64 pcpu_off_to_block_off(int off)
{
	return off & (PCPU_BITMAP_BLOCK_BITS - 1);
}

static u64 pcpu_block_off_to_off(int index, int off)
{
...
static void pcpu_block_refresh_hint(struct pcpu_chunk *chunk, int index)
{
	...
	u64 *alloc_map = pcpu_index_alloc_map(chunk, index);
 ...
 */
static struct pcpu_chunk * __init pcpu_alloc_first_chunk(u64 tmp_addr,
							 int map_size)
{
	struct pcpu_chunk *chunk;
	u64 aligned_addr, lcm_align;
...
#undef PCPU_SETUP_BUG_ON
	...
	pcpu_chunk_struct_size = sizeof(struct pcpu_chunk) +
		BITS_TO_LONGS(pcpu_unit_pages) * sizeof(u64);
	 ...
	 * the dynamic region.
	 */
	tmp_addr = (u64)base_addr + static_size;
...
static u64 pcpu_unit_page_offset(unsigned int cpu, int page_idx)
{
...
bool __is_kernel_percpu_address(u64 addr, u64 *can_addr)
{
	const size_t static_size = __per_cpu_end - __per_cpu_start;
...
static u64 pcpu_chunk_addr(struct pcpu_chunk *chunk,
				     unsigned int cpu, int page_idx)
{
	return (u64)chunk->base_addr +
...
static void __pcpu_unmap_pages(u64 addr, int nr_pages)
{
...
static int __pcpu_map_pages(u64 addr, struct page **pages,
			    int nr_pages)
{
 ...
 */
bool is_kernel_percpu_address(u64 addr)
{
...
phys_addr_t per_cpu_ptr_to_phys(void *addr)
{
	 ...
	 */
	first_low = (u64)pcpu_base_addr +
		    pcpu_unit_page_offset(pcpu_low_unit_cpu, 0);
	first_high = (u64)pcpu_base_addr +
		     pcpu_unit_page_offset(pcpu_high_unit_cpu, pcpu_unit_pages);
	if ((u64)addr >= first_low &&
	    (u64)addr < first_high) {
		for_each_possible_cpu(cpu) {
			void *start = per_cpu_ptr(base, cpu);
				  ...
				  pcpu_fc_free_fn_t free_fn)
{
	...
	if (max_distance > VMALLOC_TOTAL * 3 / 4) {
		pr_warn("max_distance=0x%llx too large for vmalloc space 0x%llx\n",
...
void __init setup_per_cpu_areas(void)
{
	...
	delta = (u64)pcpu_base_addr - (u64)__per_cpu_start;
	for_each_possible_cpu(cpu)
